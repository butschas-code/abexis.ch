...
const emptyAggregates = (): Pick<AdminDashboardData, "counts"> => ({
...
export async function loadAdminDashboardAggregates(
...
): Promise<Pick<AdminDashboardData, "counts">> {
  ...
  if (!includeSubmissionsIntake) {
    const [publishedPosts, draftPosts, categories, authors] = await Promise.all([...countQueries]);
    return {
      counts: {
        publishedPosts,
        draftPosts,
  ...
  const [publishedPosts, draftPosts, categories, authors, submissions] = await Promise.all([
    ...countQueries,
    countDocs(COLLECTIONS.submissions),
  ]);
