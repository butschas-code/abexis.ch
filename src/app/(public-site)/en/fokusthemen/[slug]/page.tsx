...
import { englishTopics, getEnglishTopicByDeSlug } from "@/data/english-site";
import { fokusPageHeroImages } from "@/data/site-images";

type Props = { params: Promise<{ slug: string }> };

export function generateStaticParams() {
  return englishTopics.map((topic) => ({ slug: topic.deSlug }));
}

export async function generateMetadata({ params }: Props) {
  const { slug } = await params;
  const topic = getEnglishTopicByDeSlug(slug);
  return {
