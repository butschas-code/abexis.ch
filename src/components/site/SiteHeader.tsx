...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-black/[0.06] bg-white">
      <div className="mx-auto flex max-w-[1068px] items-center justify-between gap-8 px-6 py-3 md:py-4">
        ...
        <button
          type="button"
          className="focus-ring rounded-full border border-black/10 bg-white px-4 py-2 text-[16px] font-medium text-[#1d1d1f] transition-colors duration-200 hover:border-black/20 hover:bg-[#f5f5f7] hover:text-brand-900 md:hidden"
          aria-expanded={open}
          onClick={() => setOpen((o) => !o)}
        >
          Menü
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <nav className="flex flex-col px-6 py-4 text-[19px] font-medium" aria-label="Hauptnavigation mobil">
              {mainNav.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
