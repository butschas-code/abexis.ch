...
export function TestimonialsSection() {
  return (
    <section className="relative overflow-hidden border-y border-black/[0.06] py-14 sm:py-20 md:py-28">
      <div
      ...
      <div className="relative mx-auto max-w-[1068px] pl-[max(1rem,env(safe-area-inset-left,0px))] pr-[max(1rem,env(safe-area-inset-right,0px))] sm:pl-6 sm:pr-6">
        ...
        <ul className="mt-10 grid list-none grid-cols-1 gap-5 sm:mt-14 sm:gap-6 md:grid-cols-2 md:gap-8">
          {homeTestimonials.map((item, idx) => (
            ...
            >
              <figure className="group flex flex-col overflow-hidden rounded-[20px] bg-white p-5 shadow-[var(--apple-shadow)] ring-1 ring-black/[0.04] transition duration-300 ease-out hover:-translate-y-0.5 hover:shadow-[var(--apple-shadow-lg)] hover:ring-black/[0.06] sm:rounded-[28px] sm:p-7 md:p-8">
                <blockquote className="relative border-l-2 border-brand-900/25 pl-3.5 sm:pl-5 md:pl-6">
                  <p className="text-[15px] font-normal leading-[1.65] text-[#3c3c43] sm:leading-relaxed sm:text-[16px]">
                    {item.quote}
                  </p>
                </blockquote>
